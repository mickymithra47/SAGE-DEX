...
contract AuthorizationFuzzTest is Test {
    ...
    // Fuzz Property: Arbitrary amounts and nonces can only execute once under valid signature
    function testFuzz_Permit2_ValidSignatureTransfer(uint256 amount, uint256 nonce) public {
        ...
        uint256 balBefore = token.balanceOf(spender);
        vm.prank(spender);
        ...
        assertEq(token.balanceOf(spender) - balBefore, amount);
    }
}
